...
<%-- Usamos la ruta absoluta tambi茅n para el CSS por si naveg谩s dentro de subrutas --%>
...
<nav class="navbar navbar-dark navbar-afa sticky-top shadow">
  <div class="container-fluid navbar-container">
    ...
    <c:if test="${sessionScope.usuario != null}">
      <span class="text-white small d-md-none">馃懁 ${sessionScope.usuario.email}</span>
    </c:if>
    <%--1. Este bot贸n solo se ve en PC gracias a d-none d-md-block --%>
  ...
  <div class="offcanvas-header border-bottom">
    ...
    <c:choose>
      <c:when test="${sessionScope.usuario != null}">
        <div>
          <h5 class="offcanvas-title fw-bold m-0">MEN脷</h5>
          <small class="text-muted">隆Hola, ${sessionScope.usuario.email}!</small>
        </div>
      </c:when>
      <c:otherwise>
        <h5 class="offcanvas-title fw-bold">MEN脷</h5>
      </c:otherwise>
    </c:choose>
  ...
  <div class="offcanvas-body p-0">
    <div class="list-group list-group-flush">
      ...
      <c:if test="${sessionScope.usuario != null}">
        ...
        <a href="${pageContext.request.contextPath}/informacionDeLaCuenta.jsp" class="list-group-item list-group-item-action">Informaci贸n de la Cuenta</a>
